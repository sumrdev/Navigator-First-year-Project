...
public class RTree<T extends Element> implements Serializable {
    ...
    protected class Node implements Serializable {
        ...
        /**
         *
         * @param values a list of elements that the Node should contain, either in
         *               further nodes or as its value
        ...
        protected float[] getNodeBounds(List<T> values) {
            float[] result = new float[] { Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY, Float.NEGATIVE_INFINITY,
         ...
         * 
         * @param values the list of values that should be split between the two child
         *               nodes
         */
         ...
         * @return the new lists, slit in the correct dimension given by the layer
         *         value, that should be used to create new child Nodes
         */
        protected List<T>[] splitList(List<T> values) {
            List<T>[] result = new List[children];
            for (int i = 0; i < children; i++) {
                result[i] = new ArrayList<>(
                        values.subList(values.size() * i / children, values.size() * (i + 1) / children));
            }
            return result;
        }
        ...
        protected void quickselect(List<T> values, int low, int high, int k) {
            ...
            if (low < high) {
                ...
                for (int i = low; i < high; i++) {
                    ...
                    // if element at index i is less than or equal to the pivot,
         ...
         */
        private void reorder(List<T> values, int a, int b) {
            try {
                T tmp = values.get(a);
         ...
         * 
         * @param rangeCoords the coordinates that all elements should be given from, as
         *                    a float array of length 4 in the order {x_min,y_min,
         *                    x_max, y_max}
        ...
        /**
         * Modifies the parameter "lowest" so that it contains the lowest element in the
         * tree
         * 
         * @param point  the point, given as a float array of length 2 in the order
         *               {x,y}
         * @param lowest an object containing the element with the lowest distance to
         *               the point seen so far
         */
        ...
        /**
         *
         * @param rect1 a float array of length 4 in the order {x_min,y_min, x_max,
         *              y_max}
         * @param rect2 a float array of length 4 in the order {x_min,y_min, x_max,
         *              y_max}
        ...
        /**
         *
         * @param rangeCoords the range, given as a float array of length 4 in the order
         *                    {x_min,y_min, x_max, y_max}
         * @return {@code true} if there's an intersection between the rangeCoords and
         *         the boundingRect of this node
         */
         ...
         * @param bounds the bounding rect given as a float array of length 4 in the
    ...
    /**
     *
     * @param box the coordinates that all elements should be given from given as a
     *            javaFX Bounds
     * @return a list containing the elements that has a bounding box that
     *         intersects with the range given
    ...
    /**
     *
     * @param range the coordinates that all elements should be given from as a
     *              float array of length 4 in the order {x_min,y_min, x_max, y_max}
     * @return a list containing the elements that has a bounding box that
     *         intersects with the range given
     */
    ...
    /**
     *
     * @param point the point, given as a Point of the type made for the map of
     *              denmark project
     * @return The element with the lowest distance to the point
    ...
    /**
     *
     * @param point the point, given as a javaFX Point2D, that should be used to get
     *              the node bounds that enclose the given point
     * @return a list of float arrays witch are the boundaries of the tree nodes
     *         that intersects the point
     * @see javafx.geometry.Point2D
     */
    public List<float[]> getElementsInRangeDebug(Point2D point) {
        float[] pointAsBound = new float[] { (float) point.getX(), (float) point.getY(), (float) point.getX(),
                (float) point.getY() };
