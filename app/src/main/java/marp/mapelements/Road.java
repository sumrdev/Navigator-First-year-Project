...
public class Road extends Element {
    ...
    public Road(long id, ArrayList<RoadNode> nodes, RoadType type, int speed, boolean oneway, boolean roundabout,
            String name) {
        this.id = id;
        this.nodes = nodes;
        this.roadType = type;
        ...
        if (name != null) {
            ...
            // determine if road is too short to have road displayed in order to avoid
            // showing too many names:
            double distanceX = lastX - firstX;
            double distanceY = lastY - firstY;
            distance = Math.sqrt(distanceX * distanceX + distanceY * distanceY);

            // calculate angle from first and last node to get approximation of slope of
            // road
    ...
    public void draw(GraphicsContext gc, int levelOfDetail, double zoom) {
        if (levelOfDetail > nodes.size())
            return;
        for (int i = 0; i < nodes.size() - levelOfDetail; i += levelOfDetail) {
            gc.strokeLine(nodes.get(i).getX(), nodes.get(i).getY(), nodes.get(i + levelOfDetail).getX(),
                    nodes.get(i + levelOfDetail).getY());
        }
        gc.strokeLine(nodes.get(nodes.size() - levelOfDetail).getX(), nodes.get(nodes.size() - levelOfDetail).getY(),
                nodes.get(nodes.size() - 1).getX(), nodes.get(nodes.size() - 1).getY());
    }
