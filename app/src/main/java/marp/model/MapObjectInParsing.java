...
public class MapObjectInParsing implements Serializable {
    ...
    public void finishPoint() {
        if (unfinishedPointType == PointType.UNDEFINED && this.completeAddressCount == 4) {
            Address address = new Address(this.street, this.housenumber, this.postcode, this.city,
                    unfinishedPoint.getX(), unfinishedPoint.getY());
            mapObjects.getAddressList().add(address);
            mapObjects.getTrie().insert(address);
        } else if (fontSize != FontSize.UNDEFINED) {
            PlaceName placeName = new PlaceName(this.name, this.fontSize, this.unfinishedPoint.getX(),
                    this.unfinishedPoint.getY());
            switch (fontSize) {
                case QUITE_SMALL:
            ...
            PointOfInterest pointOfInterest = new PointOfInterest(this.name, this.unfinishedPointType,
                    this.unfinishedPoint.getX(), this.unfinishedPoint.getY(), false);
    ...
    public void finishSimpleShape() {
        if (!isRoad) {
            ...
            if (this.unfinishedPointType != PointType.UNDEFINED) {
                // Some shapes have information that we would like to show as points. To
                // accomplish this, we make a new
                // point of interest in the middle of the shape corresponding to the type, if
                // the shape has a point type value.
                Point2D shapeCenterCoords = findCenterOfShape(coords.get(0), coords.get(1));
                PointOfInterest pointOfInterest = new PointOfInterest(this.name, this.unfinishedPointType,
                        (float) shapeCenterCoords.getX(), (float) shapeCenterCoords.getY(), false);
                mapObjects.getPOIList().add(pointOfInterest);
            }
            if (this.completeAddressCount == 4) {
                Point2D shapeCenterCoords = findCenterOfShape(coords.get(0), coords.get(1));
                Address address = new Address(street, housenumber, postcode, city, (float) shapeCenterCoords.getX(),
                        (float) shapeCenterCoords.getY());
                mapObjects.getAddressList().add(address);
                mapObjects.getTrie().insert(address);
            }
            if (this.unfinishedShapeType == ShapeType.UNDEFINED) {
                this.SimpleShapeIDToSimpleShape.put(this.unfinishedSimpleShapeID,
                        new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
            } else {
                switch (this.unfinishedShapeType) {
                    case COASTLINE:
                        coastLineSegmentList
                                .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        break;
                    case BUILDING:
                        mapObjects.getBuildingsList()
                                .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        break;
                    case WATER:
                        mapObjects.getWaterAreasList()
                                .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        break;
                    case GRASS:
                    case FOREST:
                    ...
                    case COMMERCIAL_GROUND:
                        mapObjects.getTerrainAreasList()
                                .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        break;
                    case WATERWAY:
                        mapObjects.getWaterwayList()
                                .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        break;
                    case RAILWAY:
                        if (!isTunnel) {
                            mapObjects.getRailwayList()
                                    .add(new SimpleShape(this.unfinishedShapeType, coords.get(0), coords.get(1)));
                        }
                        break;
                    default:
            ...
            if (unfinishedRoadType != RoadType.UNDEFINED) {
                if (speed == -1) {
                    // If the speed hasn't been set, we get the speed from the standard speed limits
            ...
            Road road = new Road(this.unfinishedSimpleShapeID, roadNodes, this.unfinishedRoadType, this.speed,
                    this.isOneWay, this.roundabout, this.name);
            mapObjects.getRoadsList().add(road);
            SimpleShapeIDToSimpleShape.put(this.unfinishedSimpleShapeID,
                    new SimpleShape(this.unfinishedShapeType, roadNodes));
    ...
    public void finishRelation() {
        ...
        if (unfinishedRelationSimpleShapes.size() > 0) {
            switch (unfinishedShapeType) {
                case BUILDING:
                    mapObjects.getBuildingsList()
                            .add(new ComplexShape(this.unfinishedShapeType, this.unfinishedRelationSimpleShapes));
                    break;
                case WATER:
                    mapObjects.getWaterAreasList()
                            .add(new ComplexShape(this.unfinishedShapeType, this.unfinishedRelationSimpleShapes));
